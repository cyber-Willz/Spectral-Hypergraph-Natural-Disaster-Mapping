//! Real-data benchmark: NBSC vs. GCN vs. GAT vs. GraphSAGE on the Cora
//! citation network, averaged over multiple training seeds (the
//! statistical-rigor gap the original README's "What would turn this into
//! a real paper" section flagged: "multiple seeds with confidence
//! intervals... GAT and GraphSAGE baselines").
//!
//! IMPORTANT -- read before quoting these numbers anywhere:
//! This uses the bundled `data/cora/{cora.content,cora.cites}` files and
//! the split implemented in `dataset::stratified_split`
//! (`Dataset::load_cora_default`), which has the same *shape* as the
//! standard semi-supervised Planetoid protocol (20 labeled/class train,
//! 500 val, 1000 test) but is NOT the literature's exact split (that ships
//! as Python pickles; see `dataset.rs` module docs for why it isn't
//! reproduced bit-for-bit here). Do not present these numbers next to
//! published Cora leaderboard entries as if directly comparable -- only
//! comparisons *within this run*, across models trained on the identical
//! split, are valid. Say so explicitly in the thesis text.
//!
//! Also note: `N_SEEDS` x `depths.len()` x 4 architectures full training
//! runs is a real amount of compute (each run is `EPOCHS` full-batch
//! passes over a 2708-node graph). Expect this to take noticeably longer
//! than the synthetic `benchmark` example; reduce `N_SEEDS` or the depth
//! sweep for a faster smoke test before committing to the full run you'll
//! report in the thesis.
//!
//! Run with: `cargo run --release --example benchmark_cora --features burn`
...
/// comparison) when non-zero. Defaults to 0.0 so this file, unmodified,
/// still reproduces the exact baseline results already recorded in
/// `docs/results_cora_draft.md`. To run the weight-decay experiment
/// (addresses the ~70% vs. literature's ~81.5% gap noted in that draft),
/// change this to 5e-4 -- the value from Kipf & Welling's original GCN
/// paper, widely reused as a default across the GNN literature -- and
/// rerun. That produces a new, separate experiment; it does not reproduce
/// the existing table, so don't overwrite those numbers with the new run's
/// output, add them as a new row/section.
...
fn main() {
    let ds = Dataset::load_cora_default(SPLIT_SEED).expect(
        "failed to load Cora -- check that nbsc/data/cora/{cora.content,cora.cites} exist",
    );
    println!(
        "Loaded Cora: n={}, m={}, classes={}, train={}, val={}, test={}",
        ds.graph.n,
        ds.graph.m(),
        ds.num_classes(),
    ...
    eprintln!(
        "\nRunning {} configs ({} depths x {} seeds x 4 architectures). The first layer of every \
         config does a dense 2708x2708x1433 propagation on the raw input width, so this is \
         meaningfully heavier than the synthetic benchmark -- expect real wall-clock time, not \
