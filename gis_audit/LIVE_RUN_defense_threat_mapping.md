# Live Run: Civil-Defense Threat Mapping — Texas Panhandle Wildfire Corridor
...
## Scope note: what "Defense Threat Mapping" means here

"Threat mapping" is used in the **FEMA/DHS civil-defense sense** throughout
this document — the same sense as FEMA's own THIRA process (Threat and
Hazard Identification and Risk Assessment): mapping *natural and man-made
disaster threat exposure* across a jurisdiction network for emergency
management. It is explicitly **not** military threat or targeting analysis.
Nothing in this pipeline identifies attack targets, exploitable
vulnerabilities, or anything with an offensive military use. The "threat" in
every number below is a federally-declared natural disaster — specifically,
wildfire.
...
3. **Model** — a real, trained 2-layer Graph Attention Network (`nbsc::gat_layer::GatLayer` × 2, `burn` autodiff backend), predicting `DECLARATION_COUNT` (min-max scaled) from each county's own and its neighbors' threat-exposure features. 300 epochs, full-batch Adam, learning rate 0.01.
